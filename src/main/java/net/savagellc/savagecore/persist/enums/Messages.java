package net.savagellc.savagecore.persist.enums;
...
public enum Messages {
    ...
    private String getMessage(HashMap<String, String> placeholders, boolean prefix) {
        ...
        if (isList) {
            if (exists) {
                message = Methods.pl(convertList(FileManager.Files.messages.getFile().getStringList("Messages." + path)));
            } else {
                message = Methods.pl(convertList(getDefaultListMessage()));
            }
        } else {
            if (exists) {
                message = Methods.pl(FileManager.Files.messages.getFile().getString("Messages." + path));
            } else {
